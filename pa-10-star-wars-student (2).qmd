...
> Both of these achieve the same thing, just one uses pluck while the other uses brackets.
...
> The first maps each with starships, and then gets the length of each. The second gets the length of each in the column of starshups, while the last maps it to a double and gets the length of the column of starships.
...
sw_ships %>% 
  ...
  ggplot(aes(y = paste(title, desc(episode)), 
             x = prop)) + 
  #we want bars but our data is already summarized!
  geom_bar(aes(fill = trilogy), stat = "identity") + 
  labs(
